...
🔥 **Многопротокольный MQTT брокер-ловушка и OCPP сервер** для анализа и реверс-инжиниринга IoT устройств и зарядных станций для электромобилей.
...
- **OCPP 1.6** (WS, порт 8889) - протокол зарядных станций
...
### Поддержка протокола OCPP 1.6

Брокер включает полноценный OCPP 1.6 сервер для анализа и реверс-инжиниринга зарядных станций.

### Поддерживаемые OCPP операции:
- **BootNotification** - регистрация зарядной станции
- **Heartbeat** - проверка соединения
- **StatusNotification** - статус коннекторов
...
```javascript
// Пример подключения OCPP клиента
const WebSocket = require('ws');

const ws = new WebSocket('ws://localhost:8889', ['ocpp1.6']);
...
```bash
# Используйте встроенный тестовый клиент
npm run test:ocpp
```

### Логирование OCPP

OCPP сообщения сохраняются в отдельный файл:
```
mqtt_logs/ocpp-messages.log
```

Формат сообщений OCPP:
```json
...
- `UniqueId`: Уникальный идентификатор сообщения
- `Action`: Название операции (только для Call)
...
├── ocpp-messages.log      # OCPP сообщения
